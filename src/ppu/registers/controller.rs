...
bitflags! {
    ...
    struct ControlRegisterFlags: u8 {
...
pub struct ControlRegister {
    status: ControlRegisterFlags,
}

impl ControlRegister {
    pub fn new() -> Self {
        ControlRegister {
            status: ControlRegisterFlags::from_bits_truncate(0b0000_0000),
        }
    }
    ...
    pub fn get_vram_addr_increment_value(&self) -> u8 {
        if !self
            .status
            .contains(ControlRegisterFlags::VRAM_ADD_INCREMENT)
        {
            1
        } else {
            32
